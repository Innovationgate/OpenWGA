...
public class WGDocumentImplRSS1 extends de.innovationgate.webgate.api.fake.WGFakeDocument {
	
	private String key;
	private Element item;
	private Element channel;
	private DateFormat DATE_FORMATTER = new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z");
	private Namespace dcns;
	
	public WGDocumentImplRSS1(WGDatabase db, String key, Element channel, Element item) {
		super(db, WGDocument.TYPE_CONTENT);
		this.key = key;
		this.channel = channel;
		this.item = item;
		this.dcns = this.channel.getNamespace();
	...
	public Object getMetaData(String type) throws WGSystemException, WGIllegalArgumentException, WGBackendException {
		...
		if (type.equals(WGContent.META_TITLE)) {
			...
			try {
				return this.DATE_FORMATTER.parse(this.item.elementText(new QName("date", dcns)));
			}
			...
			return this.item.elementText(new QName("creator", dcns));
		}
			...
			String lang =  this.channel.elementText(new QName("language", dcns));
