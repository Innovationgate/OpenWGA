...
/**
 * An option value provider providing the registered ways to encode passwords (modules of type {@link PasswordEncodingType})
 */
public class PasswordEncodingsValueProvider implements OptionValueProvider {
    ...
    public String getEmptyListMessage(Locale locale) {
        return "No password encodings registered"; // Cannot happen
    }

    public List<String> getProvidedValues() {
        List<String> vals = new ArrayList<String>();
        Iterator<ModuleDefinition> defs = _reg.getModulesForType(PasswordEncodingType.class).values().iterator();
        while (defs.hasNext()) {
