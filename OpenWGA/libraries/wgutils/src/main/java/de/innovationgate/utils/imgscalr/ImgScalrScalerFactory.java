...
public class ImgScalrScalerFactory implements MimeTypeSpecificImageScalerFactory {

    @Override
    public MimeTypeSpecificImageScaler getForMimeType(String mimeType) {

        if (ImgScalrScaler.SUPPORTED_MIMETYPES.contains(mimeType)) {
            return new ImgScalrScaler();
        }
        else {
            return null;
        }
